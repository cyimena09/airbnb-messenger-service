...
public interface ConversationRepository extends JpaRepository<Conversation, Integer> {
    ...
    @Query("SELECT p.conversation.id FROM Participation p " +
            ...
            "HAVING COUNT(p.conversation.id) = 2") // "2" uniquement lorsqu'une conversation est avec une autre personne et non en groupe
    Integer findConversationIdByParticipations(List<Integer> participantsIds);
